...
//   address interest;
//   //(ix) interest rate charged on loan
//   address rate;
// }
...
//   //the multiple of corepoolsize that represents the protocol's implicit valuation of the bear NFT's 
...
//   // A function to remove a specific item from the loans array
//   function removeLoan(uint _loanId) public {
...
//   // A function to remove a specific item from the auctions array
//   function removeAuction(uint _auctionId) public {
//     // Find the index of the loan to remove
//     uint index;
...
//       //maybe move this so that available capital is storage variable and constantly being updated?
...
//       //calculates 1 + ratio of total amount borrowed to core pool size, bounded at 1 & 2 (no utilisation bonus, utilisation penalty maximum 2x)
//       uint256 _ratio = Math.min(Math.max((_borrowed*10**18)/corepoolSize, 2*10**18), 1*10**18);
//       //calculates the interest rate of the loan -- apr = flat 15% rate plus 50DX/365 (where D is loan duration in days and X is ratio calculated above)
//       uint256 _interestRate = (15*10**18) + (50*_duration*ratio/365);
...
//           uint256 ratio = loan.interest*10**18/loan.borrowedAmount;
//           //updates the pending rewards (by fraction of repaid amount equal to interest/loan ratio)
//           pendingRewards += (_repayAmount*ratio)/10**18;
...
//     for (uint256 i=0; i < locks.length; i++) {
//       Lock _lock = locks[i];
//       //goes through active locks and calculates what share of the pending rewards they are entitled to
//       //first check they aren't expired
...
//       }
//   //divide the reward shares by the total weight of reward shares to get final shares
//   for(uint256 i=0; i<= length(rewardees); i++){
//     rewardShares[rewardees[i]] = rewardShares[rewardees[i]]/_totalWeight;
//     claimable[rewardess[i]] += rewardShares[rewardess[i]];
//     }
//     //pays the core pool its share of the interest 
//     corepoolSize += _remainingRewards*(corepoolSize*3/_totalWeight);
//     }
//   }
...
//     //checks the contract has enough bera and there's no bad debt
//     require(bera.balanceOf(address(this)) >= claimable[msg.sender] && badDebt == 0, "insufficient funds in contract");
//     //send them their rewards and update claimable mapping
...
//         //remove from active loans list and add to inactive loans list
//         removeLoan(_loan.loanId);
//         inactiveloans.push(_loan);
//         //initiate auction
...
//       if (auctions[i].auctionId == _auctionId){
//         //once we've found the loan, check bidder is beating current winning bid and has enough balance, and that auction isn't expired
...
// //TODO: functions for extending locks, repaying bad debt from treasury funds...